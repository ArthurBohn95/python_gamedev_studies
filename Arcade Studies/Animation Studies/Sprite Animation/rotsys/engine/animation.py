...
class AnimatedSprite(arcade.Sprite):
    ...
    def lock_on(self, target: arcade.Sprite):
        self.target = target
    ...
    def face_point(self, target: tuple[float, float] | arcade.Sprite):
        if isinstance(target, arcade.Sprite):
            point = target.position
        else:
            point = target
        
        angle = arcade.geometry_generic.get_angle_radians(
            self.center_x, self.center_y, point[0], point[1]
        ) - math.radians(90) # / 360
    ...
    def go_to(self, position: tuple[float, float], speed: float = 1, margin: float = 10) -> None:
        ...
        x_diff = position[0] - self.center_x
        y_diff = position[1] - self.center_y
    ...
    def update_direc(self) -> None:
        ...
        if self.has_target:
            self.face_point(self.target)
        elif self.cur_state in ('idle'):
            return
        else:
